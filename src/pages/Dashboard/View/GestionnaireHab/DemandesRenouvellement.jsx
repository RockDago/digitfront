...
import { FaSearch, FaUserCheck, FaEye, FaArrowLeft, FaTimes, FaFileExport, FaFileExcel, FaFilePdf, FaCheckCircle } from "react-icons/fa";
import { HiOutlineChevronLeft, HiOutlineChevronRight, HiChevronDown } from "react-icons/hi2";
import { ThemeContext } from "../../../../context/ThemeContext";

// ─── COMPOSANT MODALE D'ASSIGNATION (RENDU VIA PORTAL DANS document.body) ──
    ...
    >
      ...
      >
        <button
          onClick={onClose}
          style={{
            position: "absolute", top: "1rem", right: "1rem", color: isDark ? "#64748b" : "#94a3b8",
        ...
        >
          <option value="" disabled>-- Sélectionnez un expert --</option>
          {experts.map((expert, i) => (
            <option key={i} value={expert}>{expert}</option>
          ))}
        </select>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px" }}>
          <button
            ...
            style={{
              ...
              background: "transparent", color: isDark ? "#94a3b8" : "#64748b", border: `1px solid ${borderC}`, cursor: "pointer",
            }}
          >
            Annuler
          </button>
          <button
            ...
            style={{
              padding: "8px 16px", borderRadius: "10px", fontSize: "0.85rem", fontWeight: 600,
              background: selectedExpert ? "#3b82f6" : "#93c5fd", color: "#ffffff", border: "none",
              cursor: selectedExpert ? "pointer" : "not-allowed",
              boxShadow: selectedExpert ? "0 4px 14px rgba(59,130,246,0.35)" : "none", transition: "all 0.2s",
            }}
          >
...
function ExportMenu({ isDark }) {
  ...
  return (
    <div className="relative h-full" ref={menuRef}>
      ...
      {isOpen && (
        <div 
          // Ajout de z-[100] pour s'assurer qu'il passe au-dessus du tableau
...
// ─── COMPOSANT PRINCIPAL ──────────────────────────────────────────────────────
export default function DemandesRenouvellement() {
  ...
  const demandes = [
    { id: 12, institut: "Université d'Antananarivo", secteur: "Public",  grade: "Licence",  statut: "En attente", expiration: "2026-03-01" },
    { id: 11, institut: "ISPM",                      secteur: "Privé",   grade: "Master",   statut: "En cours",   expiration: "2026-03-10" },
    { id: 10, institut: "Université de Toamasina",   secteur: "Public",  grade: "Doctorat", statut: "En cours",   expiration: "2026-03-15" },
    { id: 9,  institut: "U-Magis",                   secteur: "Privé",   grade: "Licence",  statut: "Ajournée",   expiration: "2026-03-20" },
    { id: 8,  institut: "Université de Tuléar",      secteur: "Public",  grade: "Master",   statut: "En cours",   expiration: "2026-04-05" },
    { id: 7,  institut: "ESPA",                      secteur: "Privé",   grade: "Doctorat", statut: "En attente", expiration: "2026-04-12" },
    { id: 6,  institut: "Université de Fianarantsoa",secteur: "Public",  grade: "Licence",  statut: "En cours",   expiration: "2026-04-18" },
    { id: 5,  institut: "ESTI",                      secteur: "Privé",   grade: "Master",   statut: "Ajournée",   expiration: "2026-05-02" },
    { id: 4,  institut: "Université de Mahajanga",   secteur: "Public",  grade: "Doctorat", statut: "Validée",    expiration: "2026-05-10" },
    { id: 3,  institut: "ESstic",                    secteur: "Privé",   grade: "Licence",  statut: "En cours",   expiration: "2026-05-20" },
    { id: 2,  institut: "Université d'Antananarivo", secteur: "Public",  grade: "Master",   statut: "En attente", expiration: "2026-06-01" },
    { id: 1,  institut: "Université de Fianarantsoa",secteur: "Public",  grade: "Doctorat", statut: "Validée",    expiration: "2026-06-15" },
  ];
  ...
  const statutBadge = (statut) => {
    const map = {
      "En attente": "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-400 border border-yellow-200 dark:border-yellow-800",
  ...
  const filteredData = demandes.filter((d) => {
    const fDate = new Date(d.expiration).toLocaleDateString('fr-FR');
    const str   = `${d.id} ${d.institut} ${d.secteur} ${d.grade} ${d.statut} ${fDate}`.toLowerCase();
    const matchSearch = str.includes(search.toLowerCase());
    
    let matchDate = true;
    if (dateDebut && dateFin) matchDate = d.expiration >= dateDebut && d.expiration <= dateFin;
    else if (dateDebut) matchDate = d.expiration >= dateDebut;
    else if (dateFin)   matchDate = d.expiration <= dateFin;
  ...
  const sortedData = [...filteredData].sort((a, b) => {
    ...
    if (sortConfig.key === 'expiration') return (new Date(a.expiration) - new Date(b.expiration)) * dir;
  ...
  const handleConfirmAssign = () => {
    ...
    setToastMessage(`Renouvellement #${dossierToAssign.id} assigné à ${selectedExpert}`);
    ...
    // Réinitialisation des états avec un léger délai pour éviter un effet visuel pendant la fermeture
    setTimeout(() => {
      ...
      <div className="space-y-6 font-sans">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black tracking-tight" style={{ color: isDark ? "#f1f5f9" : "#0f172a" }}>
              Renouvellement #{viewDossier.id} — {viewDossier.institut}
            </h1>
            <p className="text-xs mt-1" style={{ color: subC }}>Détails et documents d'évaluation</p>
          </div>
        </div>
        ...
        <div className="p-6 rounded-2xl" style={{ background: isDark ? "#1e293b" : "#ffffff", border: `1px solid ${borderC}` }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-xs font-bold uppercase tracking-wider mb-4" style={{ color: subC }}>Informations générales</h3>
              <ul className="space-y-3">
                {[
                  ["Institut",       viewDossier.institut],
                  ["Secteur",        viewDossier.secteur],
                  ["Grade à renouveler", viewDossier.grade],
                  ["Date d'expiration", new Date(viewDossier.expiration).toLocaleDateString('fr-FR')],
                ].map(([label, val]) => (
                  <li key={label} className="flex justify-between">
                    <span className="text-sm" style={{ color: subC }}>{label} :</span>
                    <span className="text-sm font-semibold" style={{ color: textC }}>{val}</span>
                  </li>
                ))}
                <li className="flex justify-between items-center">
                  <span className="text-sm" style={{ color: subC }}>Statut :</span>
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-normal ${statutBadge(viewDossier.statut)}`}>
                    {viewDossier.statut}
                  </span>
                </li>
              </ul>
            ...
            <div className="border-t md:border-t-0 md:border-l pt-6 md:pt-0 md:pl-6" style={{ borderColor: borderC }}>
              <h3 className="text-xs font-bold uppercase tracking-wider mb-4" style={{ color: subC }}>Documents attachés</h3>
              <div className="flex flex-col gap-2">
                {["Rapport_Auto_Evaluation.pdf", "Preuves_Renouvellement.zip"].map((doc) => (
                  <div key={doc} className="p-3 rounded-lg flex items-center justify-between hover:bg-gray-50 dark:hover:bg-slate-800/50 transition cursor-pointer"
                    style={{ border: `1px dashed ${borderC}` }}>
                    <span className="text-sm" style={{ color: textC }}>{doc}</span>
                    <button className="text-sm font-medium" style={{ color: "#3b82f6" }}>Ouvrir</button>
                  </div>
                ))}
              </div>
            </div>
  ...
  return (
    <div className="space-y-6 font-sans relative">
      <div>
        <h1 className="text-xl font-black tracking-tight" style={{ color: isDark ? "#f1f5f9" : "#0f172a" }}>
          Liste des demandes de renouvellement d'habilitation
        </h1>
        <p className="text-xs mt-1 font-normal" style={{ color: subC }}>
          Consultez, filtrez et traitez les demandes de renouvellement reçues
        </p>
      </div>

      {/* FILTRES ET EXPORT AVEC CORRECTION Z-INDEX (z-20) */}
      <div className="flex flex-col xl:flex-row gap-3 xl:items-center relative z-20">
        ...
        <div className="relative w-full xl:max-w-[300px]">
          ...
          <input
            ...
            placeholder="Rechercher par ID, Institut, Grade..."
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 xl:pb-0 flex-1">
          {[["Expire du", dateDebut, setDateDebut], ["Au", dateFin, setDateFin]].map(([label, val, setter]) => (
            <div key={label} className="flex items-center h-full gap-2 rounded-xl px-3 py-1.5 shrink-0"
        ...
        {/* Bouton Export repoussé totalement à droite (xl:ml-auto) */}
        <div className="flex justify-end w-full xl:w-auto xl:ml-auto shrink-0">
          <ExportMenu isDark={isDark} />
        </div>
      </div>

      {/* CONTENEUR TABLEAU AVEC Z-INDEX INFÉRIEUR (z-10) */}
      <div className="flex flex-col rounded-2xl overflow-hidden relative z-10"
            ...
            <tbody>
              {pagedData.length === 0 ? (
                <tr>
                  <td colSpan="7" className="px-4 py-10 text-center" style={{ color: subC }}>
                    <div className="flex flex-col items-center gap-2 opacity-50">
                      <FaSearch size={20} />
                      <span className="text-sm font-normal">Aucune demande trouvée.</span>
                    </div>
                  </td>
                </tr>
              ) : pagedData.map((d) => (
                <tr key={d.id} className="transition-colors duration-150 cursor-default"
                  ...
                  <td className="px-4 py-4 text-center">
                    <span className={`inline-block px-2.5 py-1 rounded-md text-[10px] font-normal
                      ${d.secteur === 'Public'
                  ...
                  <td className="px-4 py-4 text-center">
                    <span className="inline-block px-2.5 py-1 rounded-full text-[10px] font-normal"
                  ...
                  <td className="px-4 py-4 text-center">
                    <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-[10px] font-normal ${statutBadge(d.statut)}`}>
                      {d.statut}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-4 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => setViewDossier(d)}
                      ...
                      <button onClick={() => handleOpenAssignModal(d)}
                        className="p-2 rounded-lg transition cursor-pointer"
                        style={{ color: "#10b981" }}
                        title="Traiter le renouvellement"
      ...
      {toastMessage && (
        <div className="fixed top-6 right-6 z-[10000] flex items-center gap-3 bg-emerald-500 text-white px-5 py-3.5 rounded-xl shadow-2xl transition-all duration-300">
